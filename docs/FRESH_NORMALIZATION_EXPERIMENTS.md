...
Training and analysis acquire an OS advisory lock on `.titan_image_writer{suffix}.lock` before reading or changing the run's checkpoint. Fork/import and initialization acquire the same ownership guard in their new destination. Different run tags have separate artifact namespaces and locks. Read-only parent import still uses strict checkpoint loading and before/after hashes.
